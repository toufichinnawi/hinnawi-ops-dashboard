...
// Create and configure the Express app (exported for Vercel serverless)
export async function createApp() {
  ...
  app.get("/api/public/pastry-production-report", async (req, res) => {
    try {
      const fromDate = String(req.query.fromDate || "");
      const toDate = String(req.query.toDate || "");
      if (!/^\d{4}-\d{2}-\d{2}$/.test(fromDate) || !/^\d{4}-\d{2}-\d{2}$/.test(toDate)) {
      ...
      const upstreamUrl = `https://operation.hjacobo.com/public/daily-orders-report-index/${fromDate}/${toDate}/pk/%20/pastry`;
      const upstream = await fetch(upstreamUrl, {
        headers: { Accept: "application/json" },
      });
      const text = await upstream.text();
      ...
      console.error("[PastryProductionReport] Failed to fetch upstream report:", err);
      res.status(502).json({ error: "Failed to fetch pastry production report" });
    }
  });

  app.get("/api/public/ck-preps-report", async (req, res) => {
    try {
      const fromDate = String(req.query.fromDate || "");
      const toDate = String(req.query.toDate || "");
      if (!/^\d{4}-\d{2}-\d{2}$/.test(fromDate) || !/^\d{4}-\d{2}-\d{2}$/.test(toDate)) {
      ...
      const upstreamUrl = `https://operation.hjacobo.com/public/daily-orders-report-index/${fromDate}/${toDate}/pk/%20/food`;
      const upstream = await fetch(upstreamUrl, {
        headers: { Accept: "application/json" },
      });
      const text = await upstream.text();
      ...
      console.error("[CKPrepsReport] Failed to fetch upstream report:", err);
      res.status(502).json({ error: "Failed to fetch CK preps report" });
    }
  });
...
// Only start the standalone server when not running as a Vercel serverless function
